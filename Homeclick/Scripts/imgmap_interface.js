...
function gui_addArea(id) {
	...
	if (document.getElementById('dd_output').value != 'css') {
		...
		temp += '<option selected="1" value="poly">polygon</option>';
	...
	temp += '</select>';
	temp += 'Rooms: <select name="img_href" class="img_href">';
	temp += '<option value="">--Choose a room--</option>';
	if (roomList != null) {
	    for (var i = 0; i < roomList.length; i++) {
	        var room = roomList[i].split('-');
	        temp += '<option value="' + room[0] + '">' + room[1] + '</option>';
	    }
	}
	temp += '</select>';
...
function gui_htmlChanged(str) {
	...
	if (document.getElementById('html_container')) {
		if (out == 'css') {
			...
			document.getElementById('html_container').value = encodeURI(str);
		}
	}
}
...
function gui_areaChanged(area) {
	...
	if (props[id]) {
		...
		if (area.ahref) { props[id].getElementsByTagName('select')[2].value = area.ahref; }
...
function gui_loadImage(src, roomStr) {
	//reset zoom dropdown
	...
	myimgmap.loadImage(src, roomStr);
}
