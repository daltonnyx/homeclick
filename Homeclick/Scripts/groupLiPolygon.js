...
fabric.GroupLiPolygon.fromURL = function(url, options, callback) {
	...
	xhr.onload = function() {
		...
		svgData.imageScale = boundaryWidth / document.getElementById('HBW').getBoundingClientRect().width;
		svgData.imageScale *= 4;
		var boundary = document.getElementById('HBF');
		...
		var g_wall = document.getElementById('HBW');
		...
		drawInlineSVG(svgData, tempCanvas.getContext('2d'), function(){
			...
			fabric.Image.fromURL(tempCanvas.toDataURL(), function(img) {
				...
				polWall.originalScale = boundaryWidth / polWall.getWidth();
